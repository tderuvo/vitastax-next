...
const acquisitions = [
  { brand: "The Bountiful Co.", sub: "Nature's Bounty, Solgar, Puritan's Pride", acquirer: "Nestlé Health Science", year: 2021, price: "$5.75B", confirmed: true, tag: "Largest supplement deal on record", tagColor: "#CA8A04" },
  { brand: "Nutrafol", sub: "Hair wellness supplements (67% stake)", acquirer: "Unilever", year: 2022, price: "~$1.2B implied", confirmed: true, tag: "DTC subscription, 100% YoY growth", tagColor: "#7C3AED" },
  { brand: "Atrium Innovations", sub: "Garden of Life, Pure Encapsulations, Klean Athlete", acquirer: "Nestlé Health Science", year: 2017, price: "$2.3B", confirmed: true, tag: "Premium/organic platform", tagColor: GREEN },
  { brand: "Blackmores", sub: "Asia-Pacific supplement leader", acquirer: "Kirin Holdings", year: 2023, price: "~$1.2B USD", confirmed: true, tag: "International expansion play", tagColor: BLUE },
  { brand: "Care/of", sub: "Personalized vitamin subscriptions", acquirer: "Bayer", year: 2020, price: "$225M", confirmed: true, tag: "Most comparable to VitaStax", tagColor: "#DC2626" },
  { brand: "Hiya Health", sub: "Children's vitamins, DTC subscription", acquirer: "USANA Health Sciences", year: 2024, price: "$261.5M", confirmed: true, tag: "DTC subscription model", tagColor: GREEN },
  { brand: "OLLY Nutrition", sub: "Gummy vitamins, accessible wellness", acquirer: "Unilever", year: 2019, price: "Undisclosed", confirmed: false, tag: "Unilever's entry into supplements", tagColor: MID },
  { brand: "Vital Proteins", sub: "America's #1 collagen brand", acquirer: "Nestlé Health Science", year: 2020, price: "Undisclosed", confirmed: false, tag: "Category dominance", tagColor: MID },
  { brand: "Persona Nutrition", sub: "AI-powered personalized vitamin subs", acquirer: "Nestlé Health Science", year: 2019, price: "Undisclosed", confirmed: false, tag: "Personalized nutrition beachhead", tagColor: MID },
  { brand: "Onnit", sub: "Performance supplements + fitness", acquirer: "Unilever", year: 2021, price: "Undisclosed", confirmed: false, tag: "Cognitive/sports niche", tagColor: MID },
];
    ...
    <div>
      <div style={{ background: NAVY, borderRadius: 10, padding: "2.5rem", marginBottom: "2rem" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.55)", margin: "0 0 1.5rem",
          fontSize: "0.9rem", lineHeight: 1.7, maxWidth: 560 }}>
          The global dietary supplement market is estimated at{" "}
          <strong style={{ color: WHITE }}>$190–210 billion in 2025</strong>, growing at
          7–9% annually — projected to exceed $400B by the mid-2030s. Nestlé, Unilever,
          Bayer, and PE have collectively spent billions acquiring supplement brands in
          the last decade.
        </p>
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
          {[
            { label: "Market size (2025)", value: "$200B+" },
            { label: "Annual growth",      value: "7–9%" },
      ...
      <Section title="Notable Acquisitions — Confirmed Deals">
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {acquisitions.map(({ brand, sub, acquirer, year, price, confirmed, tag, tagColor }) => (
            <div key={brand} style={{ background: GREY, borderRadius: 8, padding: "1rem 1.25rem" }}>
        ...
        <p style={{ fontSize: "0.72rem", color: "#9CA3AF", marginTop: "0.75rem", lineHeight: 1.6 }}>
          Confirmed prices are sourced from official press releases or SEC filings. "Undisclosed" means neither party publicly released the figure.
        </p>
      </Section>
      ...
      <div style={{ background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 8,
        ...
        <p style={{ margin: 0, fontSize: "0.9rem", color: DARK, lineHeight: 1.7 }}>
          ...
          Founded 2016, acquired by Bayer in 2020 for <strong>$225 million</strong>. The acquisition
          validated the model. However, Care/of shut down in June 2024 after Bayer cut costs.
          The lesson isn't that the model failed —{" "}
          <strong>Bayer paid $225M proving it works</strong>. The lesson is that post-acquisition
          integration risk is real, and building toward a clean, profitable exit is the smarter path
      ...
      <Section title="Who Would Buy a Brand Like VitaStax">
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {acquirers.map(({ name, deals, strategy }) => (
            <div key={name} style={{ background: GREY, borderRadius: 8, padding: "1.25rem 1.5rem" }}>
              <div style={{ fontWeight: 700, color: DARK, fontSize: "0.95rem", marginBottom: 4 }}>{name}</div>
              <div style={{ fontSize: "0.75rem", color: BLUE, fontWeight: 600, marginBottom: 8 }}>{deals}</div>
              <div style={{ fontSize: "0.88rem", color: MID, lineHeight: 1.6 }}>{strategy}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section title="What Acquirers Actually Look For">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "0.75rem" }}>
          {criteria.map(({ title, desc }) => (
            <div key={title} style={{ background: GREY, borderRadius: 8, padding: "1.25rem" }}>
              <div style={{ fontWeight: 700, color: DARK, fontSize: "0.88rem", marginBottom: 6 }}>{title}</div>
              <div style={{ fontSize: "0.82rem", color: MID, lineHeight: 1.6 }}>{desc}</div>
            </div>
          ))}
        </div>
      </Section>

      <div style={{ background: NAVY, borderRadius: 8, padding: "1.75rem 2rem" }}>
        ...
        <p style={{ color: WHITE, fontSize: "1rem", lineHeight: 1.8, margin: "0 0 0.75rem", fontWeight: 600 }}>
          The supplement acquisition market is active, well-funded, and hungry for DTC subscription brands with personalization at their core.
        </p>
        <p style={{ color: "rgba(255,255,255,0.55)", fontSize: "0.9rem", lineHeight: 1.8, margin: 0 }}>
          VitaStax doesn't need 1,000 subscribers to be interesting to a buyer — it needs a clean P&L,
          a proven subscriber model, and a brand story that a Nestlé or Unilever can take global.
          The $225M Care/of deal happened with a brand only a few years old. The category rewards
          focus, retention, and positioning — all things a bootstrapped operator can control.
        </p>
      </div>
    </div>
  );
